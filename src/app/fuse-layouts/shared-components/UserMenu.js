...
function UserMenu(props) {
	...
	return (
		<React.Fragment>
			...
			<Button className="h-64 rounded-full mr-12" onClick={userMenuClick} >
				{user.data.photoURL ?
					(
				...
				<div className="hidden sm:flex flex-col ml-12 items-start">
					...
					<Typography className="text-11 capitalize font-semibold" color="textSecondary">
			...
			>
				{!user.role || user.role.length === 0 ? (
					<React.Fragment>
						...
						<React.Fragment>
							<MenuItem component={Link} to="/personal-settings" onClick={userMenuClose} className="rounded-full">
								<ListItemIcon className="min-w-40">
