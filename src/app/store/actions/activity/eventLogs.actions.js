...
export const SET_EVENT_ACTIVITY_LOGS_LIST_UPDATING =
  '[HOMEPAGE] SET EVENT ACTIVITY LOGS UPDATING';
export const SET_EVENT_ACTIVITY_LOGS_LIST_LOADING =
  '[HOMEPAGE] SET EVENT ACTIVITY LOGS LOADING';
export const SYNC_ACTIVITY_LOGS_WITH_EVENT_ID =
  '[HOMEPAGE] SYNC EVENT ACTIVITY LOGS';
export const OPEN_EVENT_ACTIVITY_INFO_DIALOG =
  '[HOMEPAGE] OPEN EVENT ACTIVITY INFO DIALOG';
export const CLOSE_EVENT_ACTIVITY_INFO_DIALOG =
  '[HOMEPAGE] CLOSE EVENT ACTIVITY INFO DIALOG';
export const UPDATE_APPLICANT_ACTIVITY_CHECKIN_STATUS =
  '[HOMEPAGE] UPDATE APPLICANT ACTIVITY CHECKIN STATUS';
export const UPDATE_APPLICANT_ACTIVITY_REGISTRATION_STATUS =
  '[HOMEPAGE] UPDATE APPLICANT ACTIVITY REGISTRATION STATUS';
export const UPDATE_APPLICANT_PREQUESTION =
  '[HOMEPAGE] UPDATE APPLICANT PREQUESTION';
export const UPDATE_APPLICANT_REVIEWS = '[HOMEPAGE] UPDATE APPLICANT REVIEWS';

export function syncEventActivityLogs(eventId) {
  const request = axios.get(
    `${AUTH_REST_BASE_END_POINT}/api/activityLog/event/${eventId}`
  );
  return dispatch => {
...
export function updateEventReview(reviewDetail) {
  return (dispatch, getState) => {
    const applicantId = getState().auth.user.uuid;
...
export function cancelApplicant({ event, applicant }) {
  const request = axios.post(
    `${AUTH_REST_BASE_END_POINT}/api/activityLog/event/cancel`,
    formurlencoded({ event, applicant })
  );
  return dispatch => {
    if (applicant.role === 'user') {
      if (permitToSelfCancel(event)) {
        ...
        request.then(response => {
          ...
          dispatch({
            type: Actions.UPDATE_SELF_ACTIVITY_REGISTRATION_STATUS,
            payload: { registrationStatus: 'canceled' }
          });
          dispatch({
...
export function updateApplicantCheckinStatus({
  event,
  applicant,
  action = false
}) {
  const request = axios.post(
    `${AUTH_REST_BASE_END_POINT}/api/activityLog/event/checkinStatus/${action}`,
    formurlencoded({ event, applicant })
  );
  return dispatch => {
    dispatch({ type: SET_EVENT_ACTIVITY_LOGS_LIST_LOADING });
    request.then(response => {
      /* 簽到成功，更新 eventLogs */
      dispatch({
        type: Actions.UPDATE_APPLICANT_ACTIVITY_CHECKIN_STATUS,
        payload: {
          eventId: event,
          applicantId: applicant,
          checkinStatus: action
        }
      });
...
/* Call from Staff above */
export function updateApplicantRegistrationStatus({
  event,
  applicant,
  action = 'queueing'
}) {
  const request = axios.post(
    `${AUTH_REST_BASE_END_POINT}/api/activityLog/event/registration/${action}`,
    formurlencoded({ event, applicant })
  );
  return dispatch => {
    dispatch({ type: SET_EVENT_ACTIVITY_LOGS_LIST_LOADING });
