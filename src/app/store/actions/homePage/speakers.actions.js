...
export const APPEND_SPEAKER_TO_HOMEPAGE_SPEAKERS_LIST = '[HOMEPAGE] APPEND SPEAKER TO SPEAKERS LIST';
export const APPEND_NEXT_PAGE_SPEAKERS_LIST = '[HOMEPAGE] APPEND NEXT PAGE SPEAKERS';

export function syncHomePageSpeakers() {
  return dispatch => {
    dispatch({ type: SET_SPEAKERS_LIST_LOADING })
    eventBusService.getHomePageSpeakers()
      .then(response => {
...
export function syncHomePageSpeakerById(speakerId) {
  const request = axios.get(`${AUTH_REST_BASE_END_POINT}/api/speaker/${speakerId}`);
  return (dispatch) => {
    dispatch({ type: SET_SPEAKERS_LIST_LOADING })
    request
      .then((response) => {
...
export function saveSpeaker(speakerDetail) {
  return dispatch => {
    dispatch({ type: SET_SPEAKERS_LIST_LOADING })
    eventBusService.createOrUpdateSpeaker(speakerDetail)
      .then(payload => {

        dispatch(syncHomePageSpeakers())
...
export function deleteSpeaker(speakerId) {
  return dispatch => {
    dispatch({ type: SET_SPEAKERS_LIST_LOADING })
    eventBusService.deleteSpeaker(speakerId)
      .then(payload => {
