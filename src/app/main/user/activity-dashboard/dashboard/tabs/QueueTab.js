      ...
      return (
        <Link role="button" to="/events-list">
          <div className="flex flex-col justify-center items-center w-full h-full">
            <Typography variant="h3" className="text-gray-800">還沒有參加任何活動</Typography>
            <Typography variant="h3" className="text-gray-800">快來看看 YS 的新活動吧</Typography>
          </div>
        </Link>
      )
    }
    ...
    >
      <div className="widget flex w-1/2 md:w-1/4 p-12">
        <Paper className="w-full rounded-8 shadow-lg border-none text-center pt-12 pb-28">
          <Typography
            className="text-72 leading-none text-red">{200}</Typography>
          <Typography className="text-16" color="textSecondary">已參加次數</Typography>
        </Paper>
      </div>
      <div className="widget flex w-1/2 md:w-1/4 p-12">
        <Paper className="w-full rounded-8 shadow-lg border-none text-center pt-12 pb-28">
          <Typography
            className="text-72 leading-none text-green">{200}</Typography>
      ...
      <div className="widget flex w-1/2 md:w-1/4 p-12">
        <Paper className="w-full rounded-8 shadow-lg border-none text-center pt-12 pb-28">
          <Typography
            className="text-72 leading-none text-blue">{200}</Typography>
          <Typography className="text-16" color="textSecondary">報名人數</Typography>
        </Paper>
      </div>
      <div className="widget flex w-1/2 md:w-1/4 p-12">
        <Paper className="w-full rounded-8 shadow-lg border-none text-center pt-12 pb-28">
          <Typography
            className="text-72 leading-none text-orange">{200}</Typography>
          <Typography className="text-16" color="textSecondary">報名人數</Typography>
        </Paper>
      </div>
      ...
      {selfLogs.map(({ event, speaker }) => (
        <Link to={`/events-list/${event._id}`} role="button" key={event._id} className="w-full sm:w-1/2 h-320 p-6">
          <div className={clsx(classes.board, "flex flex-col items-center justify-end w-full h-full rounded pt-24 rounded-lg shadow-md hover:shadow-lg relative")} style={{ backgroundImage: `url(${imageNameToPathConverter(event.coverImageName)})` }}>
            {/* Date Label */}
            <div className={clsx(classes.dateLabel, "absolute bg-amber-600 text-white text-center px-8 rounded-full h-32")}>
              <Typography className="uppercase font-semibold tracking-wide whitespace-no-wrap text-lg leading-relaxed">
                {moment(event.startDateTime).format('MM/DD hh:mm')} - {moment(event.endDateTime).format('MM/DD hh:mm')}
              </Typography>
            </div>
            {/* Author Info */}
            <div className={clsx(classes.boardInfoWrapper, "flex justify-start items-center rounded-b-lg w-full pb-8")}>
              <Avatar src={imageNameToPathConverter(speaker.photoURL)} className="mx-10 my-5" alt={speaker.displayName} />
              <div className="flex flex-col justify-start overflow-hidden pr-12">
                ...
                <Typography className="text-16 md:text-24 font-700 whitespace-no-wrap overflow-hidden truncate" color="inherit">{event.title}</Typography>
                <Typography className={clsx(classes.boardContent, "text-16 font-700 whitespace-no-wrap overflow-hidden truncate")} color="inherit">{event.subTitle}</Typography>
              </div>
            </div>
          </div>
        </Link>
