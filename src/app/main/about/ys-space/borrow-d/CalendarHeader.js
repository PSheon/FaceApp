...
class CalendarHeader extends Toolbar {
	...
	render() {
		const { classes, mainThemeDark, label, date, WANTED_SPACE } = this.props;
					...
					<div className="flex flex-1 flex-col p-12 justify-between z-10 container">
						...
						<div className="flex flex-col items-center justify-between sm:flex-row">
							<div className="flex items-center my-16 sm:mb-0">
								...
								<FuseAnimate animation="transition.slideLeftIn" delay={300}>
									<Typography variant="h6">{spaceConverter(WANTED_SPACE)['title']} 借用</Typography>
								</FuseAnimate>
							</div>
							<div className="flex items-center">
								<Tooltip title="今天">
									<div>
										<FuseAnimate animation="transition.expandIn" delay={500}>
											<IconButton aria-label="今天" onClick={this.navigate.bind(null, navigate.TODAY)}>
												<Icon>today</Icon>
											</IconButton>
										</FuseAnimate>
									</div>
						...
						<FuseAnimate delay={500}>
							<div className="flex items-center justify-center">
								<Tooltip title="上個月">
									<IconButton aria-label="上個月" onClick={this.navigate.bind(null, navigate.PREVIOUS)}>
										<Icon>chevron_left</Icon>
									</IconButton>
								</Tooltip>
								<Typography variant="h6">{label}</Typography>
								<Tooltip title="下個月">
									<IconButton aria-label="下個月" onClick={this.navigate.bind(null, navigate.NEXT)}>
