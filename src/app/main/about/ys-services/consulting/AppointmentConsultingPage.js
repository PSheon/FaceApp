...
import ConsultingPageHeader from './ConsultingPageHeader';
...
function AppointmentConsultingPage(props) {
  ...
  const consultingLogs = CONSULTING_APPOINTMENT_LOGS['docs'];
  ...
  function consultingLogsConverter(consultingLogs) {
    return consultingLogs.map(consultingLog => {
      const startDate = consultingLog.consultingDate.split('T')[0];
      const startTime = consultingLog.consultingTimeSlot.split('-')[0];
      const startDateTime = moment(startDate + ' ' + startTime);

      return {
        id: consultingLog._id,
        title: consultingLog.consultingTopic,
        allDay: false,
        start: startDateTime.toDate(),
        end: startDateTime.add(1, 'hours').toDate()
      };
    });
  }
  ...
  function handleSubmitAppointment(event) {
    ...
    dispatch(Actions.appointmentConsulting(form));
  }
