...
import {
	Typography, Hidden, Checkbox, Table, TableBody, TableCell, TableHead, TableRow,
					...
					<TableRow>
						<TableCell className="max-w-64 w-64 p-0 text-center">
							<Checkbox
								...
								onChange={(event) => {
									event.target.checked ? dispatch(Actions.selectAllImages()) : dispatch(Actions.deSelectAllImages());
								}}
								checked={selectedImageIds.length === images.length && selectedImageIds.length > 0}
								indeterminate={selectedImageIds.length !== images.length && selectedImageIds.length > 0}
							/>
							{/* <IconButton>
						...
						<TableCell className="text-center max-w-64 w-64 p-0">
							...
							{selectedImageIds.length > 0 && (
								<ImageListMultiSelectMenu />
							)}
						</TableCell>
						<TableCell className="text-center sm:text-center">說明</TableCell>
						<TableCell className="text-center hidden md:table-cell">類型</TableCell>
						...
						<TableCell className="text-center hidden md:table-cell">大小</TableCell>
						...
						>
							<TableCell className="w-48 px-4 sm:px-12" padding="checkbox">
								<Checkbox
									...
									onChange={() => dispatch(Actions.toggleInSelectedImages(image._id))}
								/>
							</TableCell>
							...
							<TableCell className="text-center whitespace-no-wrap hidden md:table-cell">{mimnTypeConverter(image.mimeType)}</TableCell>
							<TableCell className="text-center whitespace-no-wrap hidden sm:table-cell">{image.author.displayName}</TableCell>
							<TableCell className="text-center whitespace-no-wrap hidden sm:table-cell">{image.imageHeight && image.imageWidth ? `${image.imageHeight} X ${image.imageWidth}` : '-'}</TableCell>
							<TableCell className="text-center whitespace-no-wrap hidden md:table-cell">{image.imageSize === '' ? '-' : bytesToSizeConverter(image.imageSize)}</TableCell>
							<TableCell className="text-center whitespace-no-wrap hidden sm:table-cell sm:text-left">{moment(image.updatedAt).format('LL')}</TableCell>
							<Hidden lgUp>
								<TableCell>
									{/* <IconButton
									...
									<div className="flex justify-around items-center">
										{image.imageTags.map((tag, i) =>
											<div key={tag} className={clsx(classes[tag], i !== 0 && "-ml-8", "cursor-pointer w-24 h-24 rounded-full hover:shadow-xl")}></div>
										)}
									</div>
								</TableCell>
							</Hidden>
