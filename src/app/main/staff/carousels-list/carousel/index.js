...
import {
  Button, Tab, Tabs, TextField, Icon, Typography, FormControlLabel, Switch
    ...
    if (!form) {
      if (selectedCarouselId === 'new') {
        ...
        if (CAROUSEL.docs.length) {
          const carousel = CAROUSEL.docs.filter(carousel => carousel._id === selectedCarouselId)[0]
          if (!carousel) {
            history.push({
          ...
          <div className="flex flex-1 w-full items-center justify-between">
            <div className="flex flex-col items-start max-w-full">
              <FuseAnimate animation="transition.slideRightIn" delay={300}>
                <Typography className="normal-case flex items-center sm:mb-12" component={Link} role="button" to="/staff/carousels-list" color="inherit">
            ...
            </div>
            {
              selectedCarouselId === 'new' ? (
                <FuseAnimate animation="transition.slideRightIn" delay={300}>
                  ...
                  >
                    {isLoading ? <span className="flex justify-center">新增圖片中 <LoadingSpinner width="2em" height="2em" /></span> : '新增圖片'}
                  </Button>
                </FuseAnimate>
              ) : (
                  <div>
                    <FuseAnimate animation="transition.slideRightIn" delay={300}>
                      ...
                      >
                        {isLoadingDelete ? <span className="flex justify-center">刪除圖片中 <LoadingSpinner width="2em" height="2em" /></span> : '刪除圖片'}
                      </Button>
                    </FuseAnimate>
                    <FuseAnimate animation="transition.slideRightIn" delay={300}>
                      <Button
                        ...
                        onClick={handleSaveCarousel}
                      >
                        {isLoading ? <span className="flex justify-center">更新圖片中 <LoadingSpinner width="2em" height="2em" /></span> : '更新圖片'}
                      </Button>
                    </FuseAnimate>
                  </div>
                )
                ...
                <div className="flex justify-center sm:justify-start flex-wrap">
                  ...
                  {images.map((image, key) => (
                    <div
                      ...
                      key={key}
                    >
                      <Icon className={classes.carouselImageFeaturedStar}>star</Icon>
