  ...
  return (
    <div className={clsx(classes.root, "flex flex-grow flex-shrink-0 flex-col items-center")}>
      <div className="flex flex-grow flex-shrink-0 flex-col items-center container px-16 md:px-24">
        <FuseAnimate>
          <Typography className="mt-44 sm:mt-88 sm:py-24 text-32 sm:text-40 font-300" color="inherit"> YS 活動列表</Typography>
        </FuseAnimate>
        <FuseAnimate>
        ...
        >
          <div className="w-320 h-320 p-16">
            <Link to="/staff/events-list/new" role="button" className={clsx(classes.board, classes.newBoard, "flex flex-col items-center justify-center w-full h-full rounded py-24 rounded-lg")}>
              <Icon className="text-56">add_circle</Icon>
              <Typography className="text-16 font-300 text-center pt-16 px-32" color="inherit">新增 YS 活動</Typography>
            </Link>
          </div>
          {events.map(item => (
            <Link to={`/staff/events-list/${item._id}`} role="button" key={item._id}>
              <div className="w-320 h-320 p-16">
                <div className={clsx(classes.board, !item.published && classes.unPublishedEffect, "flex flex-col items-center justify-end w-full h-full rounded pt-24 rounded-lg shadow-md hover:shadow-lg relative")} style={{ backgroundImage: `url(${imageNameToPathConverter(item.coverImageName)})` }}>
                  {/* Date Label */}
                  <div className={clsx(classes.dateLabel, "absolute bg-amber-600 text-white text-center px-8 rounded-full h-32")}>
                    <Typography className="uppercase font-semibold tracking-wide whitespace-no-wrap text-lg leading-relaxed">
                      {moment(item.startDateTime).format('LL')} - {item.published ? '已發佈' : '未發佈'}
                    </Typography>
                  </div>
                  <div className={clsx(classes.boardInfoWrapper, "flex justify-start items-center rounded-b-lg w-full py-8")}>
                    ...
                    <div className="flex flex-col justify-start overflow-hidden pr-12">
                      ...
                      <Typography className="text-16 font-300 text-gray-300 pr-32" color="inherit">{item.speaker.displayName}</Typography>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {({ isVisible }) => {
              return (
                <div className="flex justify-center items-center w-full min-h-10">
                  {isLoadingNextPageEvents && <LoadingSpinner width={128} height={128} />}
                  {isVisible && !isLoadingNextPageEvents && handleOnPageBottom()}
                </div>
              )
            }}
          </VisibilitySensor>
