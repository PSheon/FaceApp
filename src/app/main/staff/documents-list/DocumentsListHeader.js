    ...
    <div className="flex flex-col flex-1 p-8 sm:p-12 relative">
      <div className="flex items-center my-16 sm:mb-0">
        <FuseAnimate animation="transition.expandIn" delay={300}>
          <Icon className="text-32 mx-12">all_inbox</Icon>
        </FuseAnimate>
        <FuseAnimate animation="transition.slideLeftIn" delay={300}>
          <Typography variant="h6">上傳管理</Typography>
        </FuseAnimate>
      </div>
      <div className="flex flex-1 items-end">
        <FuseAnimate delay={200}>
          <Typography className="flex flex-1 pl-72 pb-12 text-16 sm:text-24">
            <span className="flex items-center">上傳 <Icon>chevron_right</Icon> 文件列表</span>
          </Typography>
        </FuseAnimate>
        ...
        <Dialog classes={{ paper: 'rounded-12' }} fullWidth fullScreen={false} open={dialogOpen} onClose={handleClose} TransitionComponent={Transition}>
          <DialogTitle id="alert-dialog-title" disableTypography className="text-center text-20 sm:text-24 font-semibold">
            上傳我的文件
          ...
          <DialogContent className="-mt-10">
            <FuseAnimate>
              <Typography className="mt-22 sm:mt-22 sm:py-12 text-14 sm:text-20 font-600 text-center pb-10" color="inherit">
                目前僅支援類型
                <br />
                <span className="text-blue">PDF</span> 和 <span className="text-blue">Word</span> 的文件
              </Typography>
            </FuseAnimate>
            ...
            <div className={clsx(classes.filePondWrapper, "mb-12 rounded-12 border-3 border-dotted")}>
              <FilePond
                ...
                maxFileSize="30MB"
                instantUpload={false}
                fileValidateTypeLabelExpectedTypesMap={{ 'application/vnd.openxmlformats-officedocument.wordprocessingml.document': '.docx', 'application/pdf': '.pdf' }}
                server={{
                  ...
                  process: (fieldName, file, metadata, load, error, progress, abort) => {
                    ...
                    request.open('POST', `${AUTH_REST_BASE_END_POINT}/api/document`);
