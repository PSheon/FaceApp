...
import { Avatar, Checkbox, Icon, IconButton, Typography, Chip } from '@material-ui/core';
...
import UserListMultiSelectMenu from './BorrowListMultiSelectMenu';
...
function BorrowListTable(props) {
	...
	function handlePageChange(wantedPageIndex) {
		if (users.length < 20 * wantedPageIndex) {
	...
	const handleChangeUserActivation = ({ _id, displayName, email, active }) => {
		dispatch(Actions.openDialog({
			children: (
				<React.Fragment>
					<DialogTitle id="alert-dialog-title" className="text-center text-lg">{active ? `禁用 ${displayName} 的帳號？` : `啟用 ${displayName} 的帳號？`}</DialogTitle>
					<DialogContent>
						<DialogContentText id="alert-dialog-description" className="font-semibold text-lg text-center">
							{active ? `請注意，您確定要 禁用 ${displayName} 的帳號 嗎？` : `請注意，您確定要 啟用 ${displayName} 的帳號 嗎？`}
						</DialogContentText>
					</DialogContent>
					<DialogActions className="justify-center">
						...
						<Button onClick={() => dispatch(Actions.toggleUserActivation({ userId: _id, email, active: !active }))} className={clsx(active ? "text-red" : "text-green", "rounded-full")}>
							確定
            </Button>
					</DialogActions>
				</React.Fragment>
				...
				columns={[
					{
						Header: () => (
							<Checkbox
								...
								indeterminate={selectedUserIds.length !== users.length && selectedUserIds.length > 0}
							/>
						),
						accessor: "",
						Cell: row => {
					...
					},
					{
						...
						Cell: row => (
							<div className="flex justify-around items-center w-full">
								<p className="font-600 text-lg"># {row.index}</p>
							...
							row.original.google == null ? (
								<div className={classes.logoImgWrapper}>
									...
									<div className="flex items-center">
										<Chip avatar={<Avatar src={socialLogoConverter('google')} className="p-2" />} label={row.original.google['displayName']} className="px-8 bg-green" />
									</div>
								)
						),
						className: "flex items-center justify-center font-bold",
						...
						Cell: row => (
							row.original.facebook == null ? (
								<div className={classes.logoImgWrapper}>
									...
									<div className="flex items-center">
										<Chip avatar={<Avatar src={socialLogoConverter('facebook')} className="p-2" />} label={row.original.facebook['displayName']} className="px-8 bg-green" />
									</div>
								)
						),
						className: "flex items-center justify-center font-bold",
