  ...
  return (
    <div className={clsx(classes.root, "flex flex-grow flex-shrink-0 flex-col items-center")}>
      <div className="flex flex-grow flex-shrink-0 flex-col items-center container px-16 md:px-24">
        <FuseAnimate>
          <Typography className="mt-44 sm:mt-88 sm:py-24 text-32 sm:text-40 font-300" color="inherit"> YS 情報列表</Typography>
        </FuseAnimate>
        <FuseAnimate>
        ...
        >
          <div className="w-320 h-320 p-16">
            <Link to="/staff/information-list/new" className={clsx(classes.board, classes.newBoard, "flex flex-col items-center justify-center w-full h-full rounded py-24 rounded-lg")} role="button">
              <Icon className="text-56">add_circle</Icon>
              <Typography className="text-16 font-300 text-center pt-16 px-32" color="inherit">新增 YS 情報</Typography>
            </Link>
          </div>
          {informationList.map(item => (
            <Link to={`/staff/information-list/${item._id}`} role="button" key={item._id}>
              <div className="w-320 h-320 p-16">
                  ...
                  <div className={clsx(classes.boardInfoWrapper, "flex justify-start items-center rounded-b-lg w-full pb-8")}>
                    ...
                    <div className="flex flex-col justify-start overflow-hidden pr-12">
                      <div className="flex justify-start">
                        ...
                        <div className={clsx(classes.boardTag, "inline-block bg-amber-600 px-2 rounded-full uppercase font-semibold tracking-wide whitespace-no-wrap mb-4 mr-8")}>{item.tags}</div>
                      </div>
