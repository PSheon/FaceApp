...
import { Typography, FormControl, FormControlLabel, Checkbox, Button } from '@material-ui/core';
...
function ButtomSection(props) {
  ...
  const eventQueueingInfos = EVENTS_ACTIVITY_LOGS.queueingInfos[selectedEventId];
  ...
  useEffect(() => {
    if (!eventActivityLogs) {
      ...
      if (eventActivityLogs.some(item => item.applicant._id === USER.uuid)) {
        const eventActivityLog = eventActivityLogs.find(activityLog => activityLog.applicant._id === USER.uuid)
  ...
  }, [USER.uuid, dispatch, eventActivityLogs, isApplied, isSyncingActivityLogs, selectedEventId])
  useEffect(() => {
    if (isApplied !== null) {
      if (eventActivityLogs.some(item => item.applicant._id === USER.uuid)) {
        const eventActivityLog = eventActivityLogs.find(activityLog => activityLog.applicant._id === USER.uuid)
  ...
  function renderApplyButton() {
    /* 已額滿 */
    if (eventQueueingInfos['succeededCount'] >= eventDetail.maximumOfApplicants) {
      return (
        <Button
          variant="contained"
          color="primary"
      ...
      return (
        <Button
          ...
          className="w-1/2 sm:w-128 mx-auto mt-16 rounded-full"
          aria-label="Register"
          disabled={!termState || !preQuestionFormChecker(form, eventDetail.preQuestionList)}
          value="legacy"
          onClick={handleSubmitApply}
          type="submit"
        >
          {isSubmitLoading ? <span className="flex justify-center">報名中 <LoadingSpinner width="2em" height="2em" /></span> : '報名活動'}
        </Button>
      )
    }
  }
    ...
    return (
      ...
      >
        {/* Applicants Count */}
        {
          (eventQueueingInfos['succeededCount'] >= eventDetail.maximumOfApplicants) ? (
            <div className="flex justify-between items-center">
              {/* 額滿 */}
              <div><Typography variant="caption">已有 {eventQueueingInfos['succeededCount']} 人報名</Typography></div>
              <div><Typography variant="caption">已額滿</Typography></div>
            </div>
          ) : (
              <div className="flex justify-between items-center">
                {/* 還有名額 */}
                <div><Typography variant="caption">已有 {eventQueueingInfos['succeededCount']} 人報名</Typography></div>
                <div><Typography variant="caption">還有 {eventDetail.maximumOfApplicants - eventQueueingInfos['succeededCount']} 個名額</Typography></div>
              </div>
            )
        }
        ...
        <BorderLinearProgress
          ...
          value={
            Math.round((eventQueueingInfos['succeededCount'] / eventDetail.maximumOfApplicants) * 100) || 0
          }
        />

        {/* Apply Button, Check has login first */}
        <Link role="button" to="/login" className="w-1/2 sm:w-200 mx-auto mt-16">
          <Button
            variant="contained"
            color="primary"
  ...
  if (!isUserDetailFinished) {
    return (
      <Link role="button" to="/personal-settings/edit" className="w-1/2 sm:w-288 mx-auto mt-16">
        <div className="flex flex-col justify-center items-center">
          <Typography variant="subtitle1" color="primary">您的資料不完整，請先完成部分資訊！</Typography>
          <Button
            variant="contained"
            color="primary"
            className="w-full rounded-full"
      ...
      <div className="w-full flex flex-col my-8">
        ...
        {!isApplied && (
          <div className="w-full flex flex-col justify-center my-4">
            ...
            <p>2.填寫報名資料不代表報名成功，報名成功者將於活動前3-5天以簡訊通知，活動當天憑報名成功簡訊入場。</p>
            <p>3.已報名成功者，若欲取消活動，請務必提前兩天來電取消 07-2313232。</p>
            <p>4.報名資料為本中心及講師為了解學員背景規劃活動內容所需，請詳細填寫。</p>
            <p>5.免費講座資源珍貴，報名後勿無故缺席，以免資源浪費，影響他人參加權益！</p>
            <p>6.參與講座請務必準時，活動開始後20分鐘不予入場。</p>
            <p>7.為維護活動品質，講座不開放現場報名、候補及旁聽。</p>
            <p>8.若需YS提供「服務學習時數認證」請於活動前告知工作同仁，並依照當天上課學習狀況，評核是否提供蓋章認證。</p>
  ...
  if (!selfEventActivityLog['checkinStatus']) {
    return (
      <div className="flex w-full flex-col justify-center items-center">
        <Typography variant="subtitle1" color="secondary">請於活動地點簽到處簽到！</Typography>
      </div>
    )
  }
