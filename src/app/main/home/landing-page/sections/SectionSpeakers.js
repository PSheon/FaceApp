    ...
    <FuseAnimate animation="transition.slideUpIn" delay={300}>
      <Slider {...settings} className={clsx(classes.sliderWrapper)}>
        {speakers.map(speaker => (
          <div className="w-320 h-320 px-4" key={speaker._id}>
            <div className={clsx(classes.board, "flex flex-col items-center justify-end w-full h-full rounded pt-24 rounded-lg shadow-md hover:shadow-lg")} style={{ backgroundImage: `url(${imageNameToPathConverter(speaker.photoURL)})` }}>
              <div className={clsx(classes.boardInfoWrapper, "flex flex-col justify-start items-center rounded-b-lg w-full pb-8 px-12")}>
                ...
                <Typography className="text-16 font-500 text-gray-300 text-center">{speaker.title}</Typography>
              </div>
            </div>
          </div>
        ))}
