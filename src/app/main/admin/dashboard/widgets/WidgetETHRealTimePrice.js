...
function WidgetETHRealTimePrice(props) {
	const theme = useTheme();
	const ETH_HISTORY = useSelector(({ markets }) => markets.eth.pricesHistory);
	const ETH_REAL_PRICE = useSelector(({ markets }) => markets.eth.price);
	const ETH_PRICE_HISTORY = useSelector(({ markets }) => markets.eth.pricesHistory);
	...
	useEffect(() => {
		if (ETH_HISTORY.length) {
			const dataAddons = {
				datasets: [
					{ label: '價格', data: _.map(ETH_HISTORY, item => item.close), fill: false }
				],
				labels: _.map(ETH_HISTORY, (item) => moment.unix(item.time).format('HH:mm')),
			}

			setOfTarget((((ETH_REAL_PRICE / ETH_PRICE_HISTORY[0].close) - 1) * 100).toFixed(2));
			setVolumn(_.reduce(ETH_HISTORY, (sum, item) => sum += Number(item.volumefrom), 0).toFixed(2));

			setCryptoData(_.assign(CONFIG_BASE, dataAddons));
		}
	}, [ETH_HISTORY, ETH_PRICE_HISTORY, ETH_REAL_PRICE]);
...
export default React.memo(WidgetETHRealTimePrice);
