...
function WidgetLTCRealTimePrice(props) {
	const theme = useTheme();
	const LTC_HISTORY = useSelector(({ markets }) => markets.ltc.pricesHistory);
	const LTC_REAL_PRICE = useSelector(({ markets }) => markets.ltc.price);
	const LTC_PRICE_HISTORY = useSelector(({ markets }) => markets.ltc.pricesHistory);
	...
	useEffect(() => {
		if (LTC_HISTORY.length) {
			const dataAddons = {
				datasets: [
					{ label: '價格', data: _.map(LTC_HISTORY, item => item.close), fill: false }
				],
				labels: _.map(LTC_HISTORY, (item) => moment.unix(item.time).format('HH:mm')),
			}

			setOfTarget((((LTC_REAL_PRICE / LTC_PRICE_HISTORY[0].close) - 1) * 100).toFixed(2));
			setVolumn(_.reduce(LTC_HISTORY, (sum, item) => sum += Number(item.volumefrom), 0).toFixed(2));

			setCryptoData(_.assign(CONFIG_BASE, dataAddons));
		}
	}, [LTC_HISTORY, LTC_PRICE_HISTORY, LTC_REAL_PRICE]);
...
export default React.memo(WidgetLTCRealTimePrice);
