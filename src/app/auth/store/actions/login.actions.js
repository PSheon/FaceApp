...
export function submitLoginGoogle(googleInfo) {
	return (dispatch) =>
		jwtService.signInWithGoogle(googleInfo)
			.then((user) => {
				dispatch(setUserData(user));
			...
			.catch(({ response }) => {
				...
				if (response.data.errors.msg.includes('E11000')) {
					dispatch({
						type: Actions.SHOW_MESSAGE,
						options: { message: `Google 信箱 ${googleInfo.googleEmail} 已被註冊` },
					});
				} else {
					dispatch({
						type: Actions.SHOW_MESSAGE,
...
export function submitLoginFacebook(facebookInfo) {
	return (dispatch) =>
		jwtService.signInWithFacebook(facebookInfo)
			.then((user) => {
				dispatch(setUserData(user));
			...
			.catch(({ response }) => {
				...
				if (response.data.errors && response.data.errors.msg.includes('E11000')) {
					dispatch({
						type: Actions.SHOW_MESSAGE,
						options: { message: `FB 信箱 ${facebookInfo.facebookEmail} 已被註冊` },
					});
				} else {
					dispatch({
						type: Actions.SHOW_MESSAGE,
...
export function submitLinkGoogle(googleInfo) {
	return (dispatch) =>
		jwtService.linkGoogle(googleInfo)
			.then((user) => {
				dispatch(setUserData(user));
...
export function submitLinkFacebook(facebookInfo) {
	return (dispatch) =>
		jwtService.linkFacebook(facebookInfo)
			.then((user) => {
				dispatch(setUserData(user));
			...
			.catch(err => {
				...
				dispatch({
					type: Actions.SHOW_MESSAGE,
					options: { message: `連結 Facebook 失敗` },
				});
			});
}
